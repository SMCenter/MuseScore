...
{
    std::string braille = "";
    if(indicator) braille.append(translate2Braille(Braille_NumIndicator.code));
...
{
    std::string braille = "";
    if(indicator) braille.append(Braille_NumIndicator.code);
...
braille_code * findBrailleCode(std::vector<braille_code *> code_lst, std::string code, bool partial_match)
{
    if(partial_match) {
        ...
        for(braille_code * br : code_lst) {
            ...
            if(codes.size() > lst.size()) return nullptr;

            for(size_t i=0; i < codes.size(); i++) {
                if(codes[i] != lst[i]) return nullptr;
            }
            return br;
        }
    } else {
        for(braille_code * br : code_lst) {
            if(br->code == code) return br;
        }
    }
    return nullptr;
}
